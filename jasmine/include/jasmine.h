...
/* LTH_M for jasmine setting */
#define OPTION_WRITE_CACHE		1   // 1 = disable, 0 = enable
#define TAG_BUFFER			1	// 1 = enable tag-based write buffer, 0 = disable
#define TAG_MAP_TABLE			0	// 1 = enable tag_based map table looging, 0 = disable
#define SYNC_EVICTION			0	// sync-intensive tag data first eviction policy
/* ------------------------  */
...
/* LTH_M for representing buffer node */
/*#if SYNC_EVICTION
typedef struct
{
	UINT32	lpn;
...
#define NUM_LSECTORS (NUM_PSECTORS / 100 * 90) // 10% provisioning
