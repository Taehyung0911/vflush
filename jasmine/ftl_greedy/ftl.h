...
//#if TAG_BUFFER
#define NUM_TAG_BUFFER		1024 // 8MB-> 1024, 16MB-> 2048, 24MB-> 3072, 32MB-> 4096
#define NUM_TAG_BUFFER_NODE	4	//((sizeof(node) * NUM_TAG_BUFFER) / BYTES_PER_PAGE) + 1
#define NUM_LOGGING_SPACE	4

#define DRAM_BYTES_OTHER	((NUM_COPY_BUFFERS + NUM_FTL_BUFFERS + NUM_HIL_BUFFERS + NUM_TEMP_BUFFERS + NUM_TAG_BUFFER + NUM_TAG_BUFFER_NODE + NUM_LOGGING_SPACE) * BYTES_PER_PAGE \
...
//#if TAG_BUFFER
#define TAG_BUFFER_PTR(BUF_ID)	(TAG_BUFFER_ADDR + ((UINT32)(BUF_ID)) * BYTES_PER_PAGE)
#define TAG_BUFFER_NODE_PTR(BUF_ID)	(TAG_BUFFER_NODE_ADDR + ((UINT32)(BUF_ID)) * sizeof(node))
#define LOGGING_SPACE_PTR(TAG, BUF_ID)	(LOGGING_SPACE_ADDR + ((UINT8)(TAG)) * BYTES_PER_PAGE + ((UINT32)(BUF_ID)) * sizeof(UINT32) * 2)
//#endif
...
//#if TAG_BUFFER																/* DRAM space allocation for tag_based write buffer */
#define TAG_BUFFER_ADDR		(WR_BUF_ADDR + WR_BUF_BYTES)
#define TAG_BUFFER_BYTES	(NUM_TAG_BUFFER * BYTES_PER_PAGE)

#define TAG_BUFFER_NODE_ADDR	(TAG_BUFFER_ADDR + TAG_BUFFER_BYTES)
#define TAG_BUFFER_NODE_BYTES	(NUM_TAG_BUFFER_NODE * BYTES_PER_PAGE)

#define LOGGING_SPACE_ADDR	(TAG_BUFFER_NODE_ADDR + TAG_BUFFER_NODE_BYTES)
#define LOGGING_SPACE_BYTES	(NUM_LOGGING_SPACE * BYTES_PER_PAGE)

#define COPY_BUF_ADDR		(LOGGING_SPACE_ADDR + LOGGING_SPACE_BYTES)
//#else
