...
static unsigned int ata_scsi_flush_xlat(struct ata_queued_cmd *qc)
{
	struct ata_taskfile *tf = &qc->tf;
	struct scsi_cmnd *scmd = qc->scsicmd;
	struct request *req = scmd ? scmd->request : NULL;
	unsigned long inum = req ? req->inum : 0;

	tf->flags |= ATA_TFLAG_DEVICE;
	tf->protocol = ATA_PROT_NODATA;
	
	if(inum != 0){
		tf->auxiliary = inum & 0xffffffff; // LTH_M
		printk(KERN_DEBUG"flush dio\n");
	}
...
static unsigned int ata_scsi_rw_xlat(struct ata_queued_cmd *qc)
{
	...
	int rc;
	u16 fp = 0;
	/* LTH_M [[ */ struct bio *bio= rq ? rq->bio : NULL;
	struct block_device *bi_bdev = bio ? bio->bi_bdev : NULL;
	dev_t bd_dev = bi_bdev ? bi_bdev->bd_dev : 0;
	unsigned long inum = 0;
	if( MAJOR(bd_dev) == 8 && MINOR(bd_dev) >= 32 ) {	
		if( bio && (bio->bi_opf & (REQ_OP_WRITE | REQ_SYNC | REQ_IDLE | REQ_DIO)) == (REQ_OP_WRITE | REQ_SYNC | REQ_IDLE | REQ_DIO) ) {
			struct dio *dio = bio->bi_private;
			if( dio != NULL && dio->op == REQ_OP_WRITE){
				if( dio->inode != NULL ){
					struct inode *inode = dio->inode;
					if(inode->i_jour)
						inum = 0;
					else
						inum = inode ? inode->i_ino : 0;
				}
			
			//printk(KERN_DEBUG"check dio write");
			}
		}
	} /* ]] */
	...
	rc = ata_build_rw_tf(inum, &qc->tf, qc->dev, block, n_block, tf_flags,
			     qc->tag, class);	
